...
- **Floating widget** — a small frameless panel showing the current state of the
  ...
  and defaults to **float on top** so it is visible on first launch; switch it
  to **pinned to desktop** from Settings or its right-click menu if you prefer
  a behind-windows gadget.
...
7. **Floating widget & tray** — a small status widget opens automatically at
   startup, **floating on top** by default. Right-click it (or use Settings
   / the tray menu) to switch between **Float on top** and **Pinned to desktop**,
   or hide it. Closing the main window keeps the app running in the system tray. Note: this is the app's own
